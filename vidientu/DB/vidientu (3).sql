...
-- Procedures
--
CREATE DEFINER=`root`@`localhost` PROCEDURE `register_account` (IN `accountId` VARCHAR(10), IN `name` VARCHAR(20), IN `username` VARCHAR(20), IN `password` VARCHAR(100), IN `bankId` VARCHAR(20), IN `nameBank` VARCHAR(20), IN `numberCard` INT(100), IN `located` VARCHAR(100))  BEGIN
    START TRANSACTION ;
    	INSERT INTO `tai_khoan`(`tai_khoan`.`MaTK`,`tai_khoan`.`TenTK`,`tai_khoan`.`username`,`tai_khoan`.`MatKhau`) VALUES(`accountId`,`name`,`username`,`password`);
        IF EXISTS (SELECT * FROM `tai_khoan` WHERE `tai_khoan`.`MaTK`=`accountId`) THEN
        INSERT INTO `the_nh`(`the_nh`.`MaThe`,`the_nh`.`TenNH`,`the_nh`.`STK`,`the_nh`.`Chinhanh`,`the_nh`.`MaTK`) VALUES (`bankId`,`nameBank`,`numberCard`,`located`,`accountId`);
        ELSE
        	ROLLBACK;
         END IF;   
...
INSERT INTO `ngan_hang` (`ID`, `Name`) VALUES
(1, 'ACB'),
(2, 'Agribank'),
(3, 'Oceanbank'),
(4, 'TPBank'),
...
('TK_09', 'nguyen a', '1991-11-11', '       ktx khu B, tx.Di An', '0123456789', '079192003393', '1', '2015-01-15', 45000000, '123456789', 'admin'),
